-- Admin registration: store requested role + require super-admin approval before login

-- 1. Approval flag (customers default approved; admins pending until approved)
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS admin_approved BOOLEAN NOT NULL DEFAULT true;

-- Existing rows stay approved (includes any seeded super admins)
UPDATE public.profiles
SET admin_approved = true
WHERE admin_approved IS DISTINCT FROM true;

-- 2. Update signup trigger: role from metadata, admin_approved for admin signups
...
BEGIN
  BEGIN
    ...
    -- requested_role: 'admin' | 'customer' | 'user' (default customer/user)

-- Gym owner onboarding fields for admin registration (steps 2–3)
-- Also ensures admin_approved exists (in case admin_registration_approval.sql was not run yet)
...
ALTER TABLE public.profiles
  ...
  ADD COLUMN IF NOT EXISTS gym_years_operating text,
  ADD COLUMN IF NOT EXISTS gym_facilities text[],
  ADD COLUMN IF NOT EXISTS gym_operating_days integer,
  ADD COLUMN IF NOT EXISTS gym_peak_hours text,
  ADD COLUMN IF NOT EXISTS gym_member_capacity text,
  ADD COLUMN IF NOT EXISTS gym_services text[];

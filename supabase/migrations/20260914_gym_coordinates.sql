-- Gym coordinates for near-me discovery
...
CREATE INDEX IF NOT EXISTS gyms_coords_idx
  ON public.gyms (latitude, longitude)
  WHERE latitude IS NOT NULL AND longitude IS NOT NULL;

-- Full sync: media + fees + coordinates
...
BEGIN
  ...
  THEN
    INSERT INTO public.gyms AS g (
      ...
      NEW.gym_latitude, NEW.gym_longitude,
      now()
    )
...
CREATE TRIGGER profiles_sync_gym
  AFTER INSERT OR UPDATE OF gym_name, gym_type, gym_city, gym_facilities, gym_services,
    ...
    gym_monthly_fee, gym_trainer_fee, gym_latitude, gym_longitude
  ON public.profiles
  FOR EACH ROW
